...
public class RunFileActivity extends Activity {
	...
	private Handler readListHandler = new Handler(){
		public void handleMessage(android.os.Message msg) {
			...
			if(info.length != 0){
				...
				runfile_left_lv.setAdapter(new RunFileAdapert(strings, RunFileActivity.this));
				readListFlag = false;
			}
		};
	};
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		try {
			...
			Toast.makeText(RunFileActivity.this, getString(R.string.wifi_error), Toast.LENGTH_SHORT).show();
		}
		...
		runfile_left_lv.setOnItemClickListener(new OnItemClickListener() {
			public void onItemClick(AdapterView<?> arg0, View arg1, int arg2,
				...
				if(wifiUtlis == null){
					try {
						wifiUtlis = new WifiUtlis(RunFileActivity.this);
						wifiUtlis.sendMessage(Utlis.sendSelectRunfileInfo((Integer)strings.get(arg2).get("id")));
		...
		runfile_right_back_btn.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				Intent intent = new Intent(RunFileActivity.this, MainActivity.class);
	...
	public boolean onKeyDown(int keyCode, KeyEvent event) {
    	if(keyCode == KeyEvent.KEYCODE_BACK){
    		AlertDialog.Builder builder = new AlertDialog.Builder(RunFileActivity.this);
