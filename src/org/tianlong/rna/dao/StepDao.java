...
public class StepDao {
	...
	public List<Step> getAllStep(Context context,int E_id){
		...
		Cursor cursor = database.rawQuery("select * from step where E_id="+E_id, null);
	...
	public boolean updateStep(Step step, Context context){
		...
		values.put("Stemp", step.getStemp());
		values.put("E_id", step.getE_id());
		long id = database.update("step", values, "S_id="+step.getS_id(), null);
		if(id != -1){
			flag = true;
		}
		database.close();
	...
	public List<DefaultStep> getAllDStep(Context context,int DE_id){
		...
		RnaOpenHelper helper = getDatebase(context);
		SQLiteDatabase database = helper.getReadableDatabase();
		Cursor cursor = database.rawQuery("select * from defaultstep where DE_id="+DE_id, null);
		while (cursor.moveToNext()) {
