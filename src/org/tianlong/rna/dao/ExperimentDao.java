...
public class ExperimentDao {
	...
	public List<DefaultExperiment> getAllDefaultExperiments(Context context){
		...
		Cursor cursor = database.rawQuery("select * from defaultexperiment", null);
	...
	//添加模板实验
	public boolean insertDefaultExperiment(DefaultExperiment experiment, Context context){
		boolean flag = false;
		RnaOpenHelper helper = getDatebase(context);
	...
	public List<Experiment> getAllExperimentsByU_id(Context context,int U_id){
		...
		RnaOpenHelper helper = getDatebase(context);
		SQLiteDatabase database = helper.getReadableDatabase();
		Cursor cursor = database.rawQuery("select * from experiment where U_id="+U_id, null);
		while (cursor.moveToNext()) {
			Experiment experiment = new Experiment();
			experiment.setE_id(cursor.getInt(cursor.getColumnIndex("E_id")));
			experiment.setEname(cursor.getString(cursor.getColumnIndex("Ename")));
			experiment.setCdate(cursor.getString(cursor.getColumnIndex("Cdate")));
			experiment.setRdate(cursor.getString(cursor.getColumnIndex("Rdate")));
			experiment.setEremark(cursor.getString(cursor.getColumnIndex("Eremark")));
			experiment.setEquick(cursor.getInt(cursor.getColumnIndex("Equick")));
			experiment.setEDE_id(cursor.getInt(cursor.getColumnIndex("EDE_id")));
	...
	public boolean updateExperiment(Experiment experiment,Context context){
		...
		values.put("EDE_id", experiment.getEDE_id());
		values.put("U_id", experiment.getU_id());
		int id = database.update("experiment", values, "E_id = "+experiment.getE_id(), null);
		if(id != -1){
			flag = true;
		}
	...
	//根据创建时间查询实验
	public Experiment getExperimentByCdate(String Cdate,Context context,int U_id){
	...
	//根据实验名称查询实验
	public Experiment getExperimentByEname(String Ename,Context context,int U_id){
		Experiment experiment = null;
		List<Experiment> list = getAllExperimentsByU_id(context,U_id);
		for (int i = 0; i < list.size(); i++) {
	...
	//查询快捷实验
	public List<Map<String, Object>> getExperimentByQuick(Context context,int U_id){
